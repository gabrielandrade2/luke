...
class EntityDisambiguationDataset(object):
    def __init__(self, dataset_dir, noise, i):
        ...
        self.test_b = load_documents_hack(
...
def load_documents_hack(csv_path, conll_path, person_names, noise_ratio):
    document_data = {}
    mention_data = load_mentions_from_csv_file(csv_path, person_names)
                ...
                if doc_mention_text == mention_text:
                    if decision(noise_ratio):
                        ...
                        if noise_type & 1:
                            try:
                                start = 1
                                while not punc_remover.sub("", document["words"][doc_mention_span["start"] - start]):
                                    start += 1
                                doc_mention_span["start"] -= start
                            except Exception:
                                pass
                        if noise_type & 2:
                            try:
                                end = 0
                                while not punc_remover.sub("", document["words"][doc_mention_span["end"] + end]):
                                    end += 1
                                end += 1
                                doc_mention_span["end"] += end
                            except Exception:
                                pass
                        doc_mention_span["text"] = " ".join(document["words"][doc_mention_span["start"]: doc_mention_span["end"]])
                    mention.update(doc_mention_span)
                    document["mentions"].append(mention)
                    mention_span_index += 1
                    break
...
def load_ppr_candidates(documents, dataset_dir):
    for document in documents:
        ...
        for mention in tqdm(document.mentions, desc="mentions", ncols=100, leave=False):
            try:
                found = False
                text = punc_remover.split(mention.text.lower())
                text.append(punc_remover.sub("", candidates[cur][0].lower()))
                while not found:
                    for t in text:
                        if t in candidates[cur][0]:
                            found = True
                            break
                    if not found:
        ...
        if len(tokens) > max_num_tokens:
            if document_split_mode == "simple":
                ...
                for mention_index, (start, end, mention) in enumerate(mention_data):
                    ...
                    input_mentions = (
                            [mention_data[mention_index]] + mention_data[:mention_index] + mention_data[
                                                                                           mention_index + 1:]
                    )
